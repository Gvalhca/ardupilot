...
void GCS_MAVLINK::send_system_time() {
    uint32_t now = AP_HAL::millis();
    if (prev_system_time_time != 0) {
        if (((now - prev_system_time_time) < 1 * 1000 / system_time_hz) || system_time_hz < 0) {
            return;
        } else {
            prev_system_time_time = now;
        }
    } else {
        prev_system_time_time = now;
    }
...
void GCS_MAVLINK::send_radio_in() {
    ...
    if (status && (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1)) {
        ...
        if (prev_rc_channels_raw_time != 0) {
            if (((now - prev_rc_channels_raw_time) < 1 * 1000 / rc_channels_raw_rate_hz) || rc_channels_raw_rate_hz < 0) {
                // return;
            } else {
    ...
    if (prev_rc_channels_time != 0) {
        if (((now - prev_rc_channels_time) < 1 * 1000 / rc_channels_rate_hz) || rc_channels_rate_hz < 0) {
            // return;
        } else {
...
void GCS_MAVLINK::send_raw_imu() {
    ...
    uint32_t now = AP_HAL::millis();
    if (prev_raw_imu_time != 0) {
        if (((now - prev_raw_imu_time) < 1 * 1000 / raw_imu_rate_hz) || raw_imu_rate_hz < 0) {
            return;
        } else {
            prev_raw_imu_time = now;
        }
    } else {
        prev_raw_imu_time = now;
    }
...
void GCS_MAVLINK::send_sensor_offsets() {
    ...
    if (prev_sensor_offsets_time != 0) {
        if (((now - prev_sensor_offsets_time) < 1 * 1000 / sensor_offsets_rate_hz) || sensor_offsets_rate_hz < 0) {
            return;
        } else {
...
void GCS_MAVLINK::send_servo_output_raw() {
    uint32_t now = AP_HAL::millis();
    if (prev_servo_output_raw_time != 0) {
        if (((now - prev_servo_output_raw_time) < 1 * 1000 / servo_output_raw_hz) || servo_output_raw_hz < 0) {
            return;
        } else {
            prev_servo_output_raw_time = now;
        }
    } else {
        prev_servo_output_raw_time = now;
    }
